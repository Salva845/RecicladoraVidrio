...
async function run() {
    ...
    if (binId) {
        ...
        const nextStatus = nextStatusByCurrent[binStatus];
    ...
    if (flowRouteId && ADMIN_TOKEN && binId && binSectorId === routeSectorId) {
    ...
    if (binId && RECOLECTOR_TOKEN && (updatedBinStatus === 'pendiente_retiro' || binStatus === 'pendiente_retiro')) {
        await request({
