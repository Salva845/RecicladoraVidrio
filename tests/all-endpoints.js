...
async function request({ name, method, path, token, body }) {
...
async function run() {
    ...
    if (binId) {
        ...
        await request({
            ...
            method: 'POST',
            path: `/api/bins/${binId}/status`,
            token: ADMIN_TOKEN,
            body: { nuevo_estado: 'pendiente', motivo: 'smoke-test' }
        });

        await request({
            name: 'POST /api/bins/:id/deactivate',
            method: 'POST',
            path: `/api/bins/${binId}/deactivate`,
            token: ADMIN_TOKEN,
            body: { motivo: 'smoke-test' }
        });
        ...
        await request({
            ...
            method: 'POST',
            path: `/api/bins/${binId}/reactivate`,
            token: ADMIN_TOKEN
        });
    }
    ...
    if (binHardware) {
        await request({
            ...
            method: 'GET',
            path: `/api/bins/hardware/${binHardware}`,
            token: ADMIN_TOKEN || RECOLECTOR_TOKEN
        });
        ...
        if (ADMIN_TOKEN) {
            await request({
                ...
                method: 'GET',
                path: `/api/events/bin/${binHardware}`,
                token: ADMIN_TOKEN
            });
            ...
            await request({
                ...
                method: 'GET',
                path: `/api/events/bin/${binHardware}/last`,
                token: ADMIN_TOKEN
            });
            ...
            await request({
                ...
                method: 'GET',
                path: `/api/events/bin/${binHardware}/stats`,
                token: ADMIN_TOKEN
            });
        }
    }
    ...
    if (binSectorId && ADMIN_TOKEN) {
        await request({
            ...
            token: ADMIN_TOKEN,
            body: {
                hardware_id: `BIN_${crypto.randomUUID().slice(0, 8)}`,
    ...
    if (binId && establecimientoId && ADMIN_TOKEN) {
        await request({
            name: 'POST /api/bins/:id/reassign',
            method: 'POST',
            path: `/api/bins/${binId}/reassign`,
            token: ADMIN_TOKEN,
            body: {
    ...
    if (routeId) {
        ...
        if (ADMIN_TOKEN && binId) {
            await request({
                name: 'POST /api/routes/:id/points',
                method: 'POST',
                path: `/api/routes/${routeId}/points`,
                token: ADMIN_TOKEN,
                body: { bote_id: binId }
            });
            ...
            await request({
                ...
                method: 'POST',
                path: `/api/routes/${routeId}/assign`,
                token: ADMIN_TOKEN,
            ...
            await request({
                ...
                method: 'POST',
                path: `/api/routes/${routeId}/cancel`,
                token: ADMIN_TOKEN,
                body: { usuario_id: adminUser?.id, motivo: 'smoke-test' }
            });
        }
        ...
        if (RECOLECTOR_TOKEN) {
            ...
            await request({
                ...
                method: 'POST',
                path: `/api/collection/routes/${routeId}/complete`,
                token: RECOLECTOR_TOKEN,
                body: { recolector_id: recolectorUser?.id }
            });
            ...
            await request({
                ...
                method: 'GET',
                path: `/api/collection/routes/${routeId}/progress`,
                token: RECOLECTOR_TOKEN
            });
        }
    }
    ...
    if (routeSectorId && ADMIN_TOKEN) {
        await request({
            ...
            body: {
                ...
                nombre: `Ruta ${crypto.randomUUID().slice(0, 6)}`
            }
        });
        ...
        await request({
            ...
            body: {
                sector_id: routeSectorId,
                creador_id: adminUser?.id,
                config: { nombre: `Ruta auto ${crypto.randomUUID().slice(0, 6)}` }
            }
        });
    }
    ...
    if (binId && RECOLECTOR_TOKEN) {
        await request({
            name: 'POST /api/collection/bins/:boteId/confirm-retirement',
            method: 'POST',
            path: `/api/collection/bins/${binId}/confirm-retirement`,
            token: RECOLECTOR_TOKEN,
            body: { recolector_id: recolectorUser?.id }
        });
    }

    if (pointId && RECOLECTOR_TOKEN) {
        await request({
    ...
    if (failed.length > 0) {
        ...
        failed.forEach((entry) => {
            console.log(` - ${entry.name} (${entry.method} ${entry.path}): ${entry.status}`);
        });
        process.exit(1);
    }
}
