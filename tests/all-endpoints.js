...
async function run() {
    ...
    let statusBinId = null;
    let statusBinHardware = null;
    let statusBinStatus = null;
    let routeBinId = null;
    let routeBinHardware = null;

    let updatedBinStatus = binStatus;

    if (binSectorId && establecimientoId && ADMIN_TOKEN) {
        const createStatusBinResponse = await request({
            name: 'POST /api/bins',
            method: 'POST',
            path: '/api/bins',
            token: ADMIN_TOKEN,
        ...
        if (createStatusBinResponse.ok) {
            statusBinId = createStatusBinResponse.body?.data?.id || null;
            statusBinHardware = createStatusBinResponse.body?.data?.hardware_id || null;
            statusBinStatus = createStatusBinResponse.body?.data?.status || null;
        }

        const createRouteBinResponse = await request({
            name: 'POST /api/bins (route)',
            method: 'POST',
            path: '/api/bins',
            token: ADMIN_TOKEN,
            body: {
                hardware_id: `ROUTE_${crypto.randomUUID().slice(0, 8).toUpperCase()}`,
                establecimiento_id: establecimientoId,
                sector_id: binSectorId,
                capacidad_litros: 120,
                tipo_vidrio: 'mixto'
        ...
        if (createRouteBinResponse.ok) {
            routeBinId = createRouteBinResponse.body?.data?.id || null;
            routeBinHardware = createRouteBinResponse.body?.data?.hardware_id || null;
        }
    }

    const statusTargetBinId = statusBinId || binId;
    const statusTargetBinStatus = statusBinStatus || binStatus;
    const statusTargetHardware = statusBinHardware || binHardware;
    ...
    const pointBinId = routeBinId || binId;
    ...
    if (flowRouteId && RECOLECTOR_TOKEN && assignOk) {
    ...
    if (routeSectorId && ADMIN_TOKEN && routeBinId) {
        await request({
